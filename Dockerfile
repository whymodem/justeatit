...
RUN apt-get update -y && apt-get install -y postgresql-client libstdc++6 openssl libncurses5 locales bash \
  && apt-get clean && rm -f /var/lib/apt/lists/*_*
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/just_eat_it ./
COPY entrypoint.sh .
